...
pub(crate) async fn run_agent_once(
    ...
    for _ in 0..max_steps {
        ...
        for call in tool_calls {
            let (output, error) = if tool_names.contains(call.name.as_str()) {
                match runtime
                    .execute_tool(&call.name, call.input.clone(), &ctx)
                    .await
                {
                ...
                (
                    None,
                    Some(format!("Tool '{}' is not registered", call.name)),
                )
            };
            ...
            messages.push(json!({
                ...
                "name": call.name,
                "content": serde_json::to_string(&tool_response).unwrap_or_else(|_| "{}".to_string()),
            }));
    ...
    messages.push(json!({
        "role": "user",
        "content": "Give the final answer now. Do not call more tools.",
    }));
...
Create or overwrite notes only when the user explicitly asks. After tool results \
arrive, answer concisely in Markdown.\n\nAvailable tools:\n{tool_names}"
    )
}
...
fn assistant_tool_call_message(message: &CompletionMessage, calls: &[ToolCall]) -> Value {
    json!({
        "role": "assistant",
